...
export const nativeTransfer = async (
  ...
  console.log(chalk.green(`AA: ${await biconomySmartAccount.getSmartAccountAddress()}`));
  ...
  try {
    ...
    relayerBal = await provider.getBalance(relayer);
    console.log(chalk.red(`After relayer: [${relayer} | ${relayerBal}]`));
  } catch (e) {
    console.log("error received ", e);
  }
};
