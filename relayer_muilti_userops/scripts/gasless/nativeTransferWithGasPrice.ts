...
export const nativeTransferWithGasPrice = async (
  ...
  const biconomyAccount = new CustomSmartAccount(biconomySmartAccountConfig, bundler);
  ...
  if (aaBalance.lt(ethers.utils.parseEther('0.02'))) {
  ...
  console.log(chalk.cyan(idx, 'Transfering ', ethers.utils.parseEther(amount.toFixed(20))));
  ...
  try {
    ...
    console.log(chalk.green(`[${idx}] userOp Hash: ${userOpResponse.userOpHash}`));
    const transactionDetails = await userOpResponse.wait();
    console.log(
      chalk.blue(`[${idx}] transactionHash: ${transactionDetails.receipt.transactionHash}`)
    );
